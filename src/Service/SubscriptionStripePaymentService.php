...
final class SubscriptionStripePaymentService
{
    ...
    public function createIntent(Plan $plan, PlanDuration $duration, User $user): array
    {
        ...
        try {
            $intent = $this->stripeClient->paymentIntents->create([
                ...
                'description' => sprintf('Abonnement %s - %s', $plan->getName(), $duration->getLabel()),
                'metadata' => [
    ...
    public function verifySucceededPayment(
        ...
        $isValid = $intent->status === 'succeeded'
            && (int) $intent->amount_received === (int) $duration->getPrice()
            && strtolower((string) $intent->currency) === $this->currency
            && (string) ($metadata['payment_kind'] ?? '') === 'subscription'
