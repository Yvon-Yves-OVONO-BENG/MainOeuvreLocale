...
final class GroupChatController extends AbstractController
{
    ...
    public function discover(): JsonResponse
    {
        ...
        /*
        * Groupes où je suis déjà membre actif.
        * On les exclut de Découvrir.
        */
        $memberships = $this->em->getRepository(ChatGroupMember::class)->findBy([
            'user' => $me,
            'status' => ChatGroupMember::STATUS_ACTIVE,
        ...
        foreach ($memberships as $membership) {
            $group = $membership->getChatGroup();

            if ($group instanceof ChatGroup && $group->getId()) {
                $memberGroupIds[] = $group->getId();
            }
        }

        /*
        * Groupes qu’on m’a recommandés.
        */
        $recommendations = $this->em
            ->getRepository(ChatGroupRecommendation::class)
            ->findBy([
                'recommendedTo' => $me,
                'status' => ChatGroupRecommendation::STATUS_PENDING,
            ]);
        ...
        foreach ($recommendations as $recommendation) {
            ...
            $recommendedGroupIds[] = $group->getId();
            $recommendationByGroupId[$group->getId()] = $recommendation;
        }

        $recommendedGroupIds = array_values(array_unique($recommendedGroupIds));

        /*
        * Afficher :
        * - groupes PUBLICS actifs
        * - OU groupes recommandés à moi, même privés/recommandables
        */
        $qb = $this->em->getRepository(ChatGroup::class)
            ->createQueryBuilder('g')
            ->andWhere('g.active = :active')
            ->andWhere(
                'g.visibility = :publicVisibility'
                . ($recommendedGroupIds !== [] ? ' OR g.id IN (:recommendedGroupIds)' : '')
            )
            ->setParameter('active', true)
            ->setParameter('publicVisibility', ChatGroup::VISIBILITY_PUBLIC)
            ->orderBy('g.createdAt', 'DESC');

        if ($recommendedGroupIds !== []) {
            $qb->setParameter('recommendedGroupIds', $recommendedGroupIds);
        }

        if ($memberGroupIds !== []) {
            $qb
                ->andWhere('g.id NOT IN (:memberGroupIds)')
                ->setParameter('memberGroupIds', $memberGroupIds);
        }
        ...
        */
        $pendingRequests = $this->em->getRepository(ChatGroupJoinRequest::class)->findBy([
            'requester' => $me,
            'status' => ChatGroupJoinRequest::STATUS_PENDING,
        ]);
        ...
        foreach ($pendingRequests as $request) {
            $group = $request->getChatGroup();

            if ($group instanceof ChatGroup && $group->getId()) {
                $pendingGroupIds[] = $group->getId();
            }
        }

        return $this->json([
            'ok' => true,
            'items' => array_map(function (ChatGroup $group) use ($me, $recommendedGroupIds, $pendingGroupIds, $recommendationByGroupId) {
                $item = $this->normalizeGroupListItem($group, $me);

                $groupId = $group->getId();
                $recommendation = $recommendationByGroupId[$groupId] ?? null;

                $isRecommended = in_array($groupId, $recommendedGroupIds, true);
                $isPublic = $group->getVisibility() === ChatGroup::VISIBILITY_PUBLIC;
                $isPrivate = $group->getVisibility() === ChatGroup::VISIBILITY_PRIVATE;

                $item['requestPending'] = in_array($groupId, $pendingGroupIds, true);
                $item['isRecommended'] = $isRecommended;
                $item['isPublic'] = $isPublic;
                $item['isPrivate'] = $isPrivate;
                $item['recommendedBy'] = $recommendation
                    ? $this->displayUserName($recommendation->getRecommendedBy())
                    : null;
                $item['note'] = $recommendation?->getNote();

                if ($isRecommended && $isPrivate) {
                    $item['displayBadge'] = 'Recommandé privé';
                } elseif ($isRecommended && $isPublic) {
                    $item['displayBadge'] = 'Recommandé public';
                } elseif ($isPublic) {
                    $item['displayBadge'] = 'Public';
                } else {
                    $item['displayBadge'] = 'Recommandé';
                }
