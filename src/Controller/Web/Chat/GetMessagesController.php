    ...
    private function getRawProfilePhoto(?User $user): ?string
{
    ...
    private function avatarUrl(Request $request, ?User $user): string
    {
        ...
        if ($photo !== '') {
            ...
            $photo = preg_replace('#^/?maindoeuvrelocale/public/#i', '', $photo);
            $photo = preg_replace('#^/?maindoeuvrelocale/#i', '', $photo);
            $photo = preg_replace('#^/?public/#i', '', $photo);
    ...
    private function absolutePublicUrl(Request $request, string $path): string
    {
        ...
        // Sécurité : jamais /public ni /maindoeuvrelocale dans l’URL finale
        $path = trim((string) $path);
        
        // Si une URL complète est stockée, on garde seulement le chemin
        $path = preg_replace('#^https?://[^/]+#i', '', $path);
        
        // Nettoyage des anciens chemins
        $path = preg_replace('#^/?maindoeuvrelocale/public/#i', '', $path);
        $path = preg_replace('#^/?maindoeuvrelocale/#i', '', $path);
        $path = preg_replace('#^/?public/#i', '', $path);
        
        // Correction des cas internes
